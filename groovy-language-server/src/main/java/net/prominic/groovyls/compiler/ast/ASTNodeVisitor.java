...
public class ASTNodeVisitor extends ClassCodeVisitorSupport {
	...
	public ASTNode getNodeAtLineAndColumn(URI uri, int line, int column) {
		...
		List<ASTNode> foundNodes = nodes.stream().filter(node -> {
			...
			Range range = Ranges.createZeroBasedRange(node.getLineNumber(), node.getColumnNumber(),
			...
			return result;
				...
				// If there is more than one result, we want the symbol whose range starts the
				// latest, with a secondary
				// sort of earliest end range.
				.sorted((n1, n2) -> Ranges.POSITION_COMPARATOR.compare(nodeToRange.get(n1).getEnd(),
						nodeToRange.get(n2).getEnd()))
				.sorted((n1, n2) -> Ranges.POSITION_COMPARATOR.reversed().compare(nodeToRange.get(n1).getStart(),
